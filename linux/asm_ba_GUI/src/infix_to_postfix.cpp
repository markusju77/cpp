...
int collectMlkm(string infix, bool debug) {
	// F(A,B,C) =
	int a = 0, b = 0, c = 0, d = 0;
	int g = infix.find("(");
	int e = infix.find(")");
	if(debug) cout << "collectMlkm, g:" << g << "e:" << e << endl;
	for (int j = g; j < e; j++) {
		if (infix[j] >= 'a' && infix[j] <= 'z' || infix[j] >= 'A' && infix[j] <= 'Z') {
			if(debug) cout << infix[j] << endl;
...
string convert(string infix, bool debug){
	int i=0;
	int a=0, b=0, c=0, d=0;
	int yksi = 0, nolla = 0;
	string postfix = "";
	...
	//while(infix[i]!='\0'){
	while(i < infix.size()){
		if(debug)	cout << "kierros:" << i << endl;
		
		if(infix[i]>='a' && infix[i]<='z' || infix[i]>='A' && infix[i]<='Z' || infix[i]=='1' || infix[i]=='0') {
			postfix += infix[i];
			if (debug) cout << "infix[i]:" << infix[i] << endl;
			/*
	...
	if(debug) cout << "Postfix is : " << postfix;
