...
std::unordered_map<int, unordered_map<int, tietue>> taulukot;
//std::unordered_map<int, tietue> taulukko;
...
bool lopputulos_SOP(int* tulos3, int pot, int muuttujatlkm) {
	...
	for (int a = pot - 1; a >= 0; a--) {
		...
		if (tulos3[a] == 1) {
			tietue t;
			vector<int> v;
			tulos4 = muunnaBinaari(muuttujatlkm, b, buffer3, false);
			cout << endl;			
...
bool tulostaPOS(int* tulos3, int pot, int muuttujatlkm) {
	...
	for (int a = pot - 1; a >= 0; a--) {		
		if (tulos3[a] == 0) {
			...
			tulos6 = muunnaBinaari(muuttujatlkm, pl2 - b, buffer3, false);
			tulos7 = muunnaBinaari(muuttujatlkm, b, buffer3, false);
...
void rakennaTaulukko2() {
	...
	for (auto it2 = iv.begin(); it2 != iv.end(); ++it2) {
		...
		v2.clear();		
		testi = it2->second;
		//cout << "taulukko.at():" << taulukko.at(testi).tunnus.at(0) << endl;
		//taulukko.at(testi).tunnus.at(0)
		...
		t2.tunnus = v2;
		//cout << "v2:" << taulukko.at(e).tunnus.at(0) << ",";
		...
		/*
		taulukko2.insert(pair<int, tietue>(e, t2));
		cout << "bin:" << taulukko2.at(ii).bin << ",";
		cout << "r_indeksi:" << taulukko2.at(ii).r_indeksi << ",";
		cout << "tunnus:" << taulukko2.at(ii).tunnus.at(0) << endl;
		
		iv2.emplace_back(t2.r_indeksi, e);
		*/
		//iv2.push_back(t2.r_indeksi, e);
		e--;
		ii++;
	}
	//exit(0);
	//iv2.emplace_back(3, 0);
	//iv2.emplace_back(2, 1);
}
...
void muuttujienAlustus() {
	...
	for (auto it14 = taulukko2.begin(); it14 != taulukko2.end(); ++it14) {
		cout << "it14: " << it14->second.tunnus.at(0) << endl;
		matriisin_otsikko[laskuri32] = it14->second.tunnus.at(0);
...
void muuttujienParitus1(int muuttujatlkm) {
	//iv2.push_back(std::make_pair(2, 0));
    //iv2.push_back(std::make_pair(3, 1));

	/*for(const auto& p : iv2) {
	...
	*/
	...
	cout << "pushing .." << endl;
	//iv2.push_back(std::make_pair(5,0));
	//iv2.push_back(std::make_pair(6,1));
...
void poistaParilliset_ja_tulosta() {
	...
	if (taulukko2_parit.empty()) {
		...
		cout << "taulukko2_parit ei ole tyhjä" << endl;
	}
...
/*
* Nyt muunnetaan tulostaSOP graafisen käyttöliittymän tarpeisiin
*/
...
void tulostaSOP(int muuttujatlkm, int pot, GtkWidget *sop) {
	...
	sop_lauseke += getF();
	gstr2 = g_strdup(sop_lauseke.c_str());
	gtk_text_buffer_set_text(buffer3, gstr2, -1);
	cout << getF();
...
void tulostaPOS2(int muuttujatlkm, int pot, GtkWidget *pos) {
	...
	GtkTextBuffer *buffer4 = gtk_text_view_get_buffer(GTK_TEXT_VIEW(pos));

	pos_lauseke += getF();
	gstr2 = g_strdup(pos_lauseke.c_str());
	gtk_text_buffer_set_text(buffer4, gstr2, -1);
	cout << getF();
	...
	if (!vektori_matriisi.empty()) {
		...
		for (auto it14 = vektori_matriisi.begin(); it14 != vektori_matriisi.end(); ++it14) {
			...
			for (int h6 = 0; h6 < muuttujatlkm; h6++) {
				...
				else if (pareja2 == 2 && vektori_matriisi.size() > 2) {
					pos_lauseke += ")";
					cout << ")";
					pari_laskuri3 = 0;
					pareja2++;
				...
				if (it14->second.bin.at(h6) != '-' && h6 < muuttujatlkm - 1 && it14->second.bin.at(h6 + 1) != '-' ||
					...
					//cout << "h5+muuttujat:" << h5 + (muuttujatlkm - 1);
					//cout << "vektori_matriisi.size():" << vektori_matriisi.size() << endl;
					if(pari_laskuri3==1 && vektori_matriisi.size() > 1){
						pos_lauseke += "+"; 
						cout << "+";
					}
				}
